...
|---|---|---|
| Meta operations: when to kill/graduate/scale an ad, fatigue, testing structure | [meta-decision-system.md](references/meta-decision-system.md) | Decision tree, ad-count ceiling, 80/20 CBO structure, fatigue bands, Advantage+ transition |
| Google: what to spend on first, structure, match types, negatives, PMax | [google-search-playbook.md](references/google-search-playbook.md) | Intent ladder, account structure, match-type gates, negatives, bidding by volume, **PMax guardrails** |
| Audience setup, copy formulas | [audience-targeting.md](references/audience-targeting.md) · [ad-copy-templates.md](references/ad-copy-templates.md) | Foundations |

**Read `daisy-paid-media` before acting on any of this.** That skill holds the
account's real numbers, the house thresholds and the spending guardrails, and it
overrides anything here that assumes a B2B lead-gen account. This one is generic
craft; that one is our account.

### Four upstream references were deliberately not installed

`b2b-paid-playbook`, `linkedin-b2b-playbook`, `abm-playbook` and
`rsa-output-spec`. Daisy Maison is UK direct-to-consumer ecommerce selling an
£11.25 personalised gift — there is no pipeline, no named-account targeting, no
LinkedIn spend and no lead scoring, and a playbook built on cost-per-lead would
quietly import the wrong unit of measurement. `conversion-tracking` and
`platform-setup-checklists` were skipped too: tracking is live through Shopify
and Windsor and the accounts have run for years.

Pull any of them from
`coreyhaines31/marketingskills/skills/ads/references/` if the business changes
shape.
